...
export class AdService {
	...
	async createAd(ad: CreateAdParams, file: Express.Multer.File) {
		...
		const imageUrl = await this.storageService.uploadImageFromBuffer(
			fileBuffer,
			`image-${ad.userId}-${Date.now()}`
		)

		const adGenerated = await this.openaiService.generateAdContent(
			ad.productName,
			ad.targetAudience,
			imageUrl
		)
		const audioPath = await this.openaiService.generateAudio(
			adGenerated.caption,
			`${ad.userId}-${Date.now()}`
		)

		const audioAzurePath = await this.storageService.uploadAudio(
			audioPath,
			`${adGenerated.title}-${ad.userId}-${Date.now()}`
		)

		const videoId = await this.videoService.createVideo(
			imageUrl,
			audioAzurePath.outputPath,
			adGenerated.title.toUpperCase(),
			`R$ ${ad.price}`
		)
